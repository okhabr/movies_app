...
export const api = `https://api.themoviedb.org/3/search/movie?api_key=${key}&language=en-US&query=`
...
export const filterMovies = (filter: FilterValues) => {
  const genresList: string = genres
    .filter((genre) => filter.genres.includes(genre.name))
    .map((genre) => genre.id)
    .join()
