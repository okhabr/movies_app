...
export const MovieCards: React.FC<MovieCardsProps> = (props) => {
    const moviesList: SearchedMovie[]  = useSelector((state: RootStore) => state.similarFilms.films);
    const result = moviesList.map( movie => <MovieCard movie={movie}/>);
    const styleClass = props.open ? style.movies__container_open : style.movies__container_close;
    return (
