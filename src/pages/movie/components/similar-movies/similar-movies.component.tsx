...
export const SimilarMovies: React.FC = (props) => {
  ...
  const result = moviesList.map((movie,index) => <MovieCard movie={movie} key={index}/>)
  const styleClass = open ? style.movies__container_open : style.movies__container_close
  return (
    <section>
          <h6 className={style.similar__header} onClick={toggle}>
            <span>Similar movies</span>
            <span className="material-icons">{`keyboard_arrow_${open ? 'up' :'down'}`}</span>
          </h6>
