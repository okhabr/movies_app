...
export const Pagination: React.FC = () => {
  ...
  const handleSetChange = (e: React.MouseEvent<HTMLElement>) => {
    ...
    if(currentSet[currentSet.length-1] >= totalPagesCount && type==='next') return;
    const offset = type === 'next' ? 5 : -5;
    const nextSet = currentSet.map(current => current + offset).filter(item => item < totalPagesCount);
  ...
  const pageItems = currentSet.map( item => <PageItem 
    currentNumber={item} 
  ...
  return (
    <div className={style.pagination__container}>
      <button data-type="previous" className={style.pagination__controller} onClick={handleSetChange}>
        <span className="material-icons">arrow_back_ios</span>
      </button>
      <ul className={style.pagination__pages}>
         {pageItems}
      </ul>
      <button data-type="next" className={style.pagination__controller} onClick={handleSetChange}>
