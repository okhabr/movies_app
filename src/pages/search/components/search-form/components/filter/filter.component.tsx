...
export const FilterForm: React.FC<FilterFormProps> = (props) => {
  ...
  const genreInputs = genres.map((genre) => (
    <label className={style.genre__label} key={genre.id}>
      {genre.name}
      <Field
        name="genres"
        type="checkbox"
  ...
  const yearLabelStyle = (offset: number | null) => ({
      ...
      {({ values }) => (
        <Form>
          <div className={style.set__container}>
            ...
            <div className={style.genre__selected_container}>
              {values.genres.map((genre, index) => (
                <div className={style.genre__selected_item} key={index}>
          ...
          <div className={style.set__container}>
            ...
            <p
              className={style.year__value}
              style={yearLabelStyle((values.year - 1990) * 10)}
            >
              {values.year}
            </p>
            <Field
